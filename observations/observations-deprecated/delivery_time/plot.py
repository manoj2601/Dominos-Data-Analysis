...
def get_delivery_time(order):
	dt1 = datetime.datetime.strptime(order['DELIVERY_DATE_TIME'], "%Y-%m-%d %H:%M:%S")
...
for order in orders:
	...
	i = getTimeIndex(dt)
	t = get_delivery_time(order)
	if (t > 60.0): # removing orders for which Delivery time is > 1hr
		continue
	finalDict[i].append(t)
...
plt.xticks(X)
plt.title(f"Avg Delivery time in each timeslot for 17-31 dec 2021 in {city}")
plt.xlabel('Time (in 24 hour format)')
plt.ylabel('Time in minutes')
plt.ylim(0, None)
# plt.savefig('individual_restaurants.png')
plt.savefig(f'./delivery_time_{city}_minutes.png')
